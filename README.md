...
### `sample_csv/`
- Contains sample CSV files generated by the application.
- These CSV files represent exported orders lists for demonstration or testing purposes.
- Example files include:
  - `orders_list(3).csv`: A sample export of order data.

### `screenshots/`
- Contains screenshots of the application to provide a visual understanding of its interface and features.
- Useful for showcasing the app in the README or during presentations.
- Example files include:
  - `dashboard_view.png`: Screenshot of the main dashboard.
  - `orders_list_view.png`: Screenshot of the orders list.
  - `order_edit.png`: Screenshot of the orders edit.
  - `order_edit_add_new_tag.png`: Screenshot of adding new tag to order.
### `src/`
- **`routes/`**: Application routes defining page views and navigation logic.
- **`utils/`**: Helper functions for processing data or interacting with APIs.
- **`styles/`**: Custom styles and theme settings for consistent design.

### `README.md`
- The main documentation file for the project.
- Provides an overview, installation instructions, and usage guidelines.

## Usage

- **Sample CSV**: Use the files in `sample_csv/` to test the export functionality or to understand the data structure.
- **Screenshots**: Check the `screenshots/` folder for a visual guide to the app. These are also useful for debugging UI issues or preparing documentation.
